<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.id" placeholder="反馈编号" clearable style="width: 160px;" class="filter-item" @keyup.enter.native="handleFilter"/>
    ...
    <el-table
      ...
      <el-table-column label="反馈编号" prop="id" sortable="reservation" align="center" min-width="105px">
      ...
      <el-table-column label="反馈时间" prop="feedbackDate" sortable="custom" min-width="115px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.feedbackDate | parseTime('{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="dialogFormVisible" title="修改反馈信息">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="反馈日期" prop="feedbackDate">
          <el-date-picker v-model="temp.feedbackDate" type="date" value-format="yyyy-MM-dd" placeholder="选择日期" style="width: 100%;"/>
        </el-form-item>
        <el-form-item label="反馈内容" prop="feedback">
          <el-input v-model="temp.feedback" :autosize="{ minRows: 2, maxRows: 10}" type="textarea" placeholder="输入活动内容" />
        </el-form-item>
        <el-form-item>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        feedbackDate: [
          { required: true, message: '请输入反馈日期', trigger: 'blur' }
        ],
        feedback: [
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateFeedback(tempData).then(response => {
            if (response.data.code === 20000) {
